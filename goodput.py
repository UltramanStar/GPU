...
class GoodputFunction(object):
    ...
    def evaluate(self, num_nodes, num_replicas, atomic_bsz, accum_steps):
        # batch_size = max(num_replicas * atomic_bsz * (accum_steps + 1),self._init_batch_size)
    ...
    def optimize(self, num_nodes, num_replicas, max_batch_size=None,
        ...
        if not np.all(np.less_equal(num_nodes, num_replicas)):
            with open('error.log','a') as f:
                info = 'node: {}, num_replicas: {}\n'.format(num_nodes,num_replicas)
                f.writelines(info)
